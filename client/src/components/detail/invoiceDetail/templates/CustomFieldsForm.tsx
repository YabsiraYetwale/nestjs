...
interface Field {
  name: string;
  value: string;
  position: number; // Add a new field to track the position
}
...
function CustomFieldsForm({ params }: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        additionalFields.forEach((item: any, index: number) => {
          if (item.additional_fields) {
            const fieldObj = item.additional_fields;
  
            Object.entries(fieldObj).forEach(([name, value]: [string, unknown]) => {
              if (name !== 'additional_fields' && name !== 'company_id' && typeof value === 'string') {
                initialFields.push({ name, value, position: index }); // Add the position to the field object
              }
            });
          }
        });
  ...
  const addField = () => {
    setFields((prevFields) => [...prevFields, { name: '', value: '', position: prevFields.length }]); // Set the position to the current length of the array
  };
